...
# --- 1. 로그인 인증 함수 ---
def check_password():
    """사용자가 올바른 암호를 입력했는지 확인합니다."""
    def password_entered():
        # Streamlit Cloud의 Secrets에 저장된 password와 비교합니다.
        # 로컬 테스트 시에는 st.secrets 대신 직접 문자열을 넣으셔도 됩니다.
    ...
    if "password_correct" not in st.session_state:
        # 로그인 화면 UI
        st.title("🔒 리모델링 분석 솔루션 보안 접속")
        st.text_input("접속 암호를 입력하세요", type="password", on_change=password_entered, key="password")
        st.info("이 도구는 승인된 내부 관계자만 이용할 수 있습니다.")
        return False
    elif not st.session_state["password_correct"]:
        st.title("🔒 리모델링 분석 솔루션 보안 접속")
        st.text_input("접속 암호를 입력하세요", type="password", on_change=password_entered, key="password")
        st.error("⚠️ 암호가 틀렸습니다. 다시 입력해 주세요.")
        return False
    else:
        return True

# --- 2. 메인 애플리케이션 (인증 성공 시 실행) ---
if check_password():
    # [1. 유틸리티 함수]
    def clean_value(val):
    ...
    if uploaded_file:
        ...
        st.subheader("📊 1. 보정된 항목별 실시간 가중치")
        st.info("💡 **가중치 산출 알고리즘:** Sheet1 공사비를 분석하여 항목 간 중복도를 코사인 유사도로 측정하고, 독립성이 높은 항목에 가중치를 부여했습니다.")
        w_df = pd.DataFrame([{"항목": k, "독립성 지수(f_j)": round(f_index[i], 4), "최종 가중치(W_j)": round(WEIGHTS_MAP[k], 4)} for i, k in enumerate(ITEMS)]).set_index("항목")
        st.table(w_df.sort_values(by="최종 가중치(W_j)", ascending=False).T)
        ...
        if 'res' in st.session_state:
            ...
            st.success(f"✅ 분석 결과, 입력하신 조건과 가장 유사도가 높은 단지는 **'{best_project}'** 입니다. (유사도 점수: {res_df.iloc[0]['유사도 점수(%)']:.2f}%)")
            st.divider()
            st.subheader("🔍 4. 상세 분석 (4단계 차이 계산 근거)")
            selected_project = st.selectbox("상세 분석 단지 선택", res_df.index)
            breakdown = []
            for item in ITEMS:
                s = calc_map[item][selected_project]
                breakdown.append({"항목": item, "가중치": round(WEIGHTS_MAP[item], 4), "전 차이": round(s[0], 6), "후 차이": round(s[1], 6), "증감 차이": round(s[2], 6), "평균 차이율": round(s[3], 6), "기여도(%)": round((s[4] / total_scores[selected_project]) * 100, 2)})
            st.table(pd.DataFrame(breakdown).sort_values(by="기여도(%)", ascending=False))
            st.info("**💡 계산 원리 설명**\n- **전/후 차이**: 입력값 대비 DB값의 수치적 차이 비율입니다.\n- **증감 차이**: 리모델링 변화량(증가분)에 대한 차이 비율입니다.\n- **기여도**: 해당 항목의 차이가 전체 유사도 점수를 결정하는 데 미친 영향력입니다.")
